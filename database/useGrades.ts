...
export async function addGradesToDatabase(
  grades: SharedGrade[],
  subject: string
) {
  const db = getDatabaseInstance();
  for (const item of grades) {
    const id = generateId(
      item.createdByAccount + item.description + item.givenAt
    );

    const existing = await db
      .get("grades")
      .query(Q.where("gradeId", id))
      .fetch();
...
export async function getGradePeriodsFromCache(
  ...
  try {
    ...
    if (periodgrades.length === 0) {
      return {
        studentOverall: { value: NaN, disabled: false },
        classOverall: { value: NaN, disabled: false },
        subjects: [],
        modules: [],
      };
    }
