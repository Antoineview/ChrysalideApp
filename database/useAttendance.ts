...
import {
  mapAbsencesToShared,
  mapDelaysToShared,
  mapObservationsToShared,
  mapPunishmentsToShared,
} from "./mappers/attendance";
import {
  Absence,
  Attendance,
  Delay,
...
export async function addAttendanceToDatabase(
  attendances: SharedAttendance[],
  period: string
) {
  const db = getDatabaseInstance();
  for (const attendance of attendances) {
    const id = generateId(
      attendance.createdByAccount + period + attendance.kidName
    );
    ...
    await safeWrite(
      ...
      async () => {
        ...
        if (existing.length > 0) {
          ...
          await Promise.all([
            ...(await existingAttendance.delays.fetch()).map((d: Delay) =>
              d.markAsDeleted()
            ),
            ...(await existingAttendance.absences.fetch()).map((a: Absence) =>
              a.markAsDeleted()
            ),
            ...(await existingAttendance.observations.fetch()).map(
              (o: Observation) => o.markAsDeleted()
            ),
            ...(await existingAttendance.punishments.fetch()).map(
              (p: Punishment) => p.markAsDeleted()
            ),
          ]);
          ...
          for (const punishment of attendance.punishments) {
            await db.get("punishments").create(record => {
              Object.assign(record, {
                ...
                homeworkDocumentsRaw: JSON.stringify(
                  punishment.homework.documents ?? []
                ),
                reasonDocumentsRaw: JSON.stringify(
                  punishment.reason.documents ?? []
                ),
...
export async function getAttendanceFromCache(
  ...
  try {
    ...
    if (!attendance[0]) {
      // Attendance not implemented yet - return undefined silently
      return undefined;
    }
    ...
    // Silently return undefined - attendance is not implemented yet
    return undefined;
  }
}
